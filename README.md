...
- **Multi-Range Trimming** — Keep one or more time ranges from the source video and have them concatenated together in the export. Use the **+ Add Range** button in the Trim inspector to create extra ranges (e.g. "Verse 1" + "Chorus" + "Final hook"). Each range has its own start/end controls, ±0.1s/±1s nudge buttons, and a "play this range" jump shortcut. During playback, the player automatically skips from the end of one range to the start of the next so you can preview the concatenated output before exporting. The export pipeline trims and crops each range separately, then stitches them via FFmpeg's `concat` demuxer (no re-encode), and lyric block timing is remapped across the concatenated output (blocks straddling range boundaries appear in each overlapping segment). Single-range projects still work exactly as before — the legacy fast path is preserved
- **Crossfade Between Ranges** — When you have 2+ trim ranges, the Trim inspector exposes a **Crossfade** slider (0–2 s) that adds a smooth video/audio fade between consecutive segments at export time. Implemented with FFmpeg's `xfade` (video) + `acrossfade` (audio) filter chain — the export re-encodes the joined output, total duration shrinks by `(N-1) × crossfade`, and lyric block timing is shifted earlier to match the overlap. Set to `0` (default) to keep the original hard-cut behavior. Preview playback still uses hard cuts — the crossfade is applied at export only
- **Output Aspect Ratio Presets** — Pick the output canvas shape from four presets: **9:16** (Reels/Shorts/TikTok, 1080×1920), **4:5** (Instagram feed portrait, 1080×1350), **1:1** (square, 1080×1080), and **16:9** (YouTube horizontal, 1920×1080). Combine with the resolution picker to choose **1080p** (full quality) or **720p** (smaller files). Default is 9:16 / 1080p, matching previous releases. The crop pipeline, subtitle renderer, and preview all use the same dynamic canvas size — what you see is what you get
- **Vertical / Horizontal Reels Modes** — Two crop layouts that work with any output aspect:
  - **세로모드 (Vertical)** — Cover-mode crop to fill the canvas with horizontal/vertical offset sliders and 1x–3x zoom. No black bars
...
- **Watermark / Logo Overlay** — Pin a PNG/JPEG logo to any of the four canvas corners with adjustable size (5–50% of canvas width), opacity (10–100%), and pixel margins (0–400 px) on the X/Y axes. The image is embedded as PNG bytes inside the `.mreels` project file (max 5 MB) so projects stay self-contained. Watermark renders below the metadata overlay and subtitles, so creator branding never blocks lyrics
...
- **Unified Preview/Export Rendering** — Both preview and export use the same Core Graphics subtitle renderer (`SubtitleRenderer`), rendering at the configured output canvas (e.g. 1080×1920 for 9:16 / 1080p) with multi-pass outline/shadow/fill. Preview displays a scaled-down version, ensuring pixel-identical typography across aspect-ratio presets
- **Two-Stage Export** — Stage 1: FFmpeg trim + crop/scale to the chosen output canvas (H.264 CRF 18, AAC 192k). Stage 2: AVAssetReader/Writer frame-by-frame burn-in of metadata overlay + subtitle CGImage overlays
...
- **Subtitle File Import (LRC / SRT)** — Bring in pre-aligned lyrics from an existing LRC or SRT file via the `+` menu in the Lyrics panel. SRT cues map to single blocks (first line = primary, second line = secondary). LRC entries with identical timestamps are paired into one bilingual block; missing end times are inferred from the next entry's start (last cue gets +5s). Imported blocks are flagged as fully manually adjusted (auto-anchor, 100% confidence) so they qualify as trusted anchors for piecewise correction. Round-trips with the LRC/SRT exporter
